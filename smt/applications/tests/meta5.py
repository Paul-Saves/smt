...
v = []
w = []
for (xi, yi,zi) in itertools.product(np.linspace(0, 5, 6),np.linspace(0, 5, 6),  np.linspace(-5, -2, 3)):
    u.append(xi)
    v.append(yi)
    w.append(zi)
x_cont = np.concatenate(
...
for (xi, yi,zi,ai) in itertools.product(np.linspace(0, 5, 6),np.linspace(0, 5, 6),np.linspace(0, 5, 6),  np.linspace(-5, -2, 3)):
    u.append(xi)
    v.append(yi)
    w.append(zi)
    x.append(ai)
x_cont = np.concatenate(
    (np.asarray(x).reshape(-1,1),np.asarray(w).reshape(-1,1), np.asarray(v).reshape(-1, 1), np.asarray(u).reshape(-1, 1)), axis=1
)
